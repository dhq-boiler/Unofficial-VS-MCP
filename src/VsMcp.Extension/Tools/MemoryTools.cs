...
namespace VsMcp.Extension.Tools
{
    public static class MemoryTools
    {
        public static void Register(McpToolRegistry registry, VsServiceAccessor accessor)
        {
            registry.Register(
                new McpToolDefinition(
                    "memory_read",
                    "Read memory bytes at a given address expression. Uses pointer arithmetic via the debugger evaluator. Must be in break mode.",
                    SchemaBuilder.Create()
                        .AddString("address", "Address expression (e.g. '0x7fff1234', '&myVariable', 'pBuffer')", required: true)
                        .AddInteger("count", "Number of bytes to read (default: 64, max: 1024)")
                        .Build()),
                args => MemoryReadAsync(accessor, args));

            registry.Register(
                new McpToolDefinition(
                    "memory_read_variable",
                    "Get a variable's memory address and raw byte representation. Must be in break mode.",
                    SchemaBuilder.Create()
                        .AddString("variable", "Variable name to inspect (e.g. 'myStruct', 'myArray[0]')", required: true)
                        .Build()),
                args => MemoryReadVariableAsync(accessor, args));
        }

        private static async Task<McpToolResult> MemoryReadAsync(VsServiceAccessor accessor, JObject args)
        {
            var address = args.Value<string>("address");
            if (string.IsNullOrEmpty(address))
                return McpToolResult.Error("Parameter 'address' is required");
            ...
            return await accessor.RunOnUIThreadAsync(() =>
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    // Try to parse for ASCII representation
        ...
        private static async Task<McpToolResult> MemoryReadVariableAsync(VsServiceAccessor accessor, JObject args)
        {
            var variable = args.Value<string>("variable");
            if (string.IsNullOrEmpty(variable))
                return McpToolResult.Error("Parameter 'variable' is required");
            ...
            return await accessor.RunOnUIThreadAsync(() =>
            {
                ...
                if (size <= 0) size = 16; // default fallback
                ...
                // Read raw bytes if we have an address
                var bytes = new List<string>();
